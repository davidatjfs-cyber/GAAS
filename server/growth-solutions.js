...
import {
  ...
  ymd,
} from './domains/growth-solutions/metrics-helpers.js';
...
⚠️ 【极其重要，实测发现过真实错误】如果上面的老板问题下方出现了"【真实数据·...】"这个标记开头的区块，
说明系统已经查到了这家店的真实数据（哪怕数字是0或很小），这种情况下analysis字段绝对禁止出现"当前无本店
真实数据"这类说法——你必须引用区块里的具体数字（比如"在职X人/离职Y人"、"近30天差评N条"），"当前无本店
真实数据，以下基于行业经验判断"这句话只允许在老板问题下方完全没有出现任何"【真实数据·...】"区块时使用，
不能因为区块里的数字本身不够全面（比如没有"近30天流失率"这种趋势数据，只有"当前在职/离职人数"这种快照）
就当作"没有数据"处理——快照数据也是真实数据，必须使用。`;
