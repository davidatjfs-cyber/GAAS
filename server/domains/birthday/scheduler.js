...
async function runBirthdayGreetingTickForTenant(deps, tenantId) {
  const {
    getSharedState,
    saveSharedState,
...
export async function runBirthdayGreetingTick(deps) {
  const { runForActiveTenants } = deps;
  try {
    await runForActiveTenants(async (tenantId) => {
      try {
        await runBirthdayGreetingTickForTenant(deps, tenantId);
      } catch (e) {
        log.error({
...
export function startBirthdayGreetingScheduler(deps) {
  let started = false;
  return function start() {
    if (started) return;
    started = true;
    setInterval(() => {
      runBirthdayGreetingTick(deps).catch((e) =>
...
export function createBirthdayScheduler(deps) {
  const ctx = {
    ...deps,
    getNow: deps.getNow || (() => new Date()),
  };
  return {
    runBirthdayGreetingTick: () => runBirthdayGreetingTick(ctx),
    startBirthdayGreetingScheduler: startBirthdayGreetingScheduler(ctx),
  };
}
