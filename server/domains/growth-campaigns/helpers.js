...
// 覆盖中文报错与错误码：黑名单管控 / 用户已退订营销短信 / 手机号码格式错误 /
// 空号 / 号码状态错误 / 号码不存在。易失性错误（余额不足、网络、模板缺失、AccessKey 风险）不算。
// 注意：业务停机（停机）不在此列——2026-08-04 核实该报错曾在 6-09/6-29/6-30/7-01/7-13 整批出现，
// 3915 个号码中 3858 个事后成功收到短信，属批次/上游异常而非号码失效，不能据此自动抑制。
const SMS_PERMANENT_FAIL_RE =
  /空号|黑名单|退订|号码状态错误|号码不存在|手机号(?:码)?格式错误|MOBILE_NUMBER_ILLEGAL|MOBILE_NUMBER_NULL|BLACK_KEY_CONTROL_LIMIT|BLACK_USER_CONTROL_LIMIT/i;
