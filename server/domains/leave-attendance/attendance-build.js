...
export function buildAttendanceFromReports(clampNum, items) {
  const out = [];
  const map = new Map();
...
export function buildAttendanceFromCheckinRecords(deps, rows, options = {}) {
  const { safeDateOnly, isLegacyTestUsername } = deps;
  const out = [];
  const map = new Map();
...
export function buildAttendanceSummaryRows(registerRows, checkinDetails) {
...
export function createAttendanceBuildHelpers({ clampNum, safeDateOnly, isLegacyTestUsername }) {
  const deps = { safeDateOnly, isLegacyTestUsername };
  return {
    buildAttendanceFromReports: (...args) => buildAttendanceFromReports(clampNum, ...args),
    isCountableCheckinStatus,
    shanghaiDateOnly,
    buildAttendanceFromCheckinRecords: (...args) => buildAttendanceFromCheckinRecords(deps, ...args),
