/**
 * P5.4 peel: recurring reward scheduler helpers.
 */
import { childLogger } from '../../utils/logger.js';

const log = childLogger({ domain: 'approvals', handler: 'scheduler-recurring-reward' });

export function shanghaiCalendarForJobs(now = new Date()) {
...
export async function insertRewardPunishmentApprovalFromTemplate(deps, applicantUsername, payloadObj) {
  const {
    pool,
    getSharedState,
    stateFindUserRecord,
...
async function notifyRecurringRewardAssignee(deps, item, genPayload, applicantUsername, currentAssignee, state, applicant) {
  const {
    saveSharedState,
    stateFindUserRecord,
  ...
  try {
    ...
    (async () => {
      try {
        ...
        log.error({
          msg: 'recurring_reward_feishu_notify_failed',
          err: feishuErr?.message || String(feishuErr),
        });
      }
    })();
  } catch (ne) {
...
export async function runMonthlyRecurringRewardTemplatesJob(deps, slotState) {
  const {
    pool,
    getActiveTenantIds,
    tenantContext,
  } = deps;
  const cal = shanghaiCalendarForJobs(deps.getNow ? deps.getNow() : new Date());
  if (cal.d !== 10 || cal.hour !== 7 || cal.minute >= 20) return;
  const slotKey = `${cal.ymd}_rrt`;
  if (slotState.lastSlot === slotKey) return;
  slotState.lastSlot = slotKey;
    ...
    await tenantContext.run(tplTenantId, async () => {
      ...
      try {
        ...
        const { item, currentAssignee, state, applicant } =
          await insertRewardPunishmentApprovalFromTemplate(deps, applicantUsername, genPayload);
        await notifyRecurringRewardAssignee(
          deps,
          item,
          genPayload,
          applicantUsername,
        ...
        log.error({
          msg: 'recurring_reward_template_failed',
          template_id: tpl.id,
