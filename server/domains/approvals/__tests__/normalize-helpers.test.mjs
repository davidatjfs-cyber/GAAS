...
test('normalizeApprovalType accepts whitelist and rejects invalid', () => {
  ...
  assert.equal(normalizeApprovalType('offboarding'), 'offboarding');
  assert.equal(normalizeApprovalType('LEAVE'), 'leave');
  assert.equal(normalizeApprovalType('payment'), 'payment');
  assert.equal(normalizeApprovalType('reward_punishment'), 'reward_punishment');
  assert.equal(normalizeApprovalType('points'), 'points');
...
test('getPaymentFlowForStore handles missing map, empty approvers, cashier trim', () => {
  ...
  assert.deepEqual(getPaymentFlowForStore('not-object', 'A'), { approvers: [], cashier: '' });
  assert.deepEqual(getPaymentFlowForStore({ paymentFlowByStore: null }, 'A'), { approvers: [], cashier: '' });
  assert.deepEqual(getPaymentFlowForStore({ paymentFlowByStore: 'bad' }, 'A'), { approvers: [], cashier: '' });
  assert.deepEqual(
...
test('normalizePromotionTrainingPeriods default title uses 1-based index', () => {
  const { normalizePromotionTrainingPeriods } = createApprovalNormalizeHelpers({
    safeDateOnly,
    randomUUID: () => 'u1',
  });
  ...
  assert.equal(out[0].title, '培训周期1');
});

test('normalizePromotionTrainingPeriods skips non-object rows', () => {
  const { normalizePromotionTrainingPeriods } = createApprovalNormalizeHelpers({
    safeDateOnly,
    randomUUID: () => 'uuid-skip',
  });
  assert.deepEqual(
    normalizePromotionTrainingPeriods([42, { startDate: '2026-01-01', endDate: '2026-01-01', title: 'ok' }]),
    [{ id: 'uuid-skip', title: 'ok', startDate: '2026-01-01', endDate: '2026-01-01', note: '' }]
  );
});
