...
export function createTryHandleBiByFunctionCalling(deps) {
  ...
  return async function tryHandleBiByFunctionCalling({ text, store, brand, senderRole, senderUsername }) {
    ...
    if (!safeStore || safeStore === '总部') { log.info({ msg: 'bi_fc', detail: ['[bi-fc] skip: no valid store'] }); return null; }
    ...
    log.info({ msg: 'bi_fc', detail: ['[bi-fc] start intent planning for:', JSON.stringify(text).slice(0, 80), 'store:', safeStore, 'historyTurns:', convHistory.length / 2] });
    ...
    if (intentPlan.intent === 'marketing_plan_request') {
      ...
      try {
        ...
        await recordAiInteraction(pool(), {
          ...
          tenantId: marketingTenantId,
        }).catch((error) => log.error({ msg: 'bi_fc', detail: ['[bi-fc] marketing quality trace failed:', error?.message || error] }));
        log.info({ msg: 'bi_fc', detail: ['[bi-fc] marketing_plan_request: generated real plan, len:', planResponse.length] });
        return {
    ...
    if (!preferredTool) { log.info({ msg: 'bi_fc', detail: ['[bi-fc] skip: no tool for intent', intentPlan.intent] }); return null; }
    ...
    if (!budgetExceeded) {
      ...
      log.info({ msg: 'bi_fc', detail: ['[bi-fc] toolPlanner ok:', toolPlanner?.ok, 'toolCalls:', toolCalls.length, 'content:', String(toolPlanner?.content || '').slice(0, 80)] });
    ...
    if (!name) { log.info({ msg: 'bi_fc', detail: ['[bi-fc] skip: no tool name resolved'] }); return null; }
    ...
    log.info({ msg: 'bi_fc', detail: ['[bi-fc] executed ok:', executed?.ok, 'source:', executed?.source, 'textLen:', String(executed?.text || '').length] });
    if (!executed?.text) { log.info({ msg: 'bi_fc', detail: ['[bi-fc] skip: empty tool result'] }); return null; }
