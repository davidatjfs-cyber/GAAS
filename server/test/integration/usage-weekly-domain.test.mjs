...
test('GET /api/admin/usage-weekly：无 token → 401', async () => {
  const res = await fetch(app.baseUrl + '/api/admin/usage-weekly');
  assert.equal(res.status, 401);
});

test('GET /api/admin/usage-weekly：store_employee → 403', async () => {
  const username = uniqueId('uw_emp');
...
test('GET /api/admin/usage-weekly：admin → 200 + period 与 data 数组', async () => {
  const username = uniqueId('uw_admin');
  ...
  const body = await res.json();
  assert.equal(res.status, 200, JSON.stringify(body));
  assert.ok(typeof body.periodStart === 'string' && body.periodStart.length >= 10);
  assert.ok(typeof body.periodEnd === 'string' && body.periodEnd.length >= 10);
  assert.ok(Array.isArray(body.data), JSON.stringify(body).slice(0, 300));
});
