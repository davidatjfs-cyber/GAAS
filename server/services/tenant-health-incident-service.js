...
import {
  ...
  healNotifyCustomer,
  healNotifyOps,
} from './tenant-health-incident/notification-actions.js';
...
export async function healIncident(pool, incidentId, { action } = {}) {
  ...
  try {
    ...
    else if (actionId === 'notify_customer') result = await healNotifyCustomer(pool, incident, _notifiers);
    else if (actionId === 'notify_ops') result = await healNotifyOps(pool, incident, _notifiers);
    else if (actionId === 'audit_delivery_failures') result = await healAuditDeliveryFailures(pool, incident, _notifiers);
