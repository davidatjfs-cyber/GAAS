...
const FIELD_DEFS = {
  ...
  orderType: { label: '订单类型', aliases: ['订单类型', '堂食外卖', '订单来源', '就餐方式', '消费类型', 'order_type', 'source'] },
...
function inferMapping(headers, sampleRows) {
  ...
  for (const field of ['phone', 'bizDate', 'checkoutRaw', 'amount', 'orderNo', 'dish', 'store', 'diners', 'orderType', 'memberName', 'tableNo', 'qty', 'category']) {
...
function normalizeWorkbook(filePath) {
  const workbook = XLSX.readFile(filePath, { cellDates: true });
  const orders = new Map();
  const itemsByOrder = new Map();
  const diagnostics = { sheets: [], missing_required: [], warnings: [], confidence_score: 0 };
  for (const table of worksheetTables(workbook)) {
    ...
    for (const row of dataRows) {
      ...
      const amount = num(cell(row, mapping, 'amount'));
      ...
      if (dish && (orderNo || phone || bizDate)) {
        const itemKey = orderNo || `${phone || 'unknown'}:${bizDate}:${amount}`;
      ...
      if (!orderNo && !phone && !amount && !bizDate) continue;
      if (!hasOrderSignal && dish) continue;
      const key = orderNo || `${phone || 'unknown'}:${bizDate}:${amount}:${orders.size}`;
  ...
  const normalized = Array.from(orders.values()).filter((o) => o.bizDate || o.amount || o.phone);
...
function analyzeOrders(orders, opts = {}) {
  ...
  const valid = orders.filter((o) => o.amount > 0 || o.phone);
  ...
  for (const o of valid) {
    ...
    const id = o.phone || `anonymous:${o.orderNo}`;
  ...
  return {
    ...
    business: {
      ...
      orders: valid.length,
      customers: customerRows.length,
      avg_check: valid.length ? Math.round(totalRevenue / valid.length * 100) / 100 : 0,
...
export function registerCustomerOpsRoutes(app, pool, authRequired, upload, uploadsDir, recordUploadOwnership) {
  app.post('/api/customer-ops/diagnosis/upload', authRequired, upload.single('file'), async (req, res) => {
    try {
      ...
      const normalized = normalizeWorkbook(req.file.path);
      const orders = Array.isArray(normalized) ? normalized : normalized.orders;
      const report = analyzeOrders(orders, { storeName: req.body?.store_name || '', diagnostics: normalized.diagnostics || {} });
      const ins = await pool.query(
        ...
        [req.tenantId || 'default', report.store_name, req.file.originalname || req.file.filename, JSON.stringify(report), req.user?.username || '']
      );
      ...
      for (const c of report.customers) {
        await pool.query(
          ...
          [req.tenantId || 'default', diagnosisId, c.customer_id, c.customer_key, c.phone || '', JSON.stringify(c)]
        );
      }
      res.json({ ok: true, diagnosis_id: diagnosisId, report: { ...report, customers: undefined } });
    } catch (e) {
