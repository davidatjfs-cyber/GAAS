...
DO $$
BEGIN
  IF to_regclass('public.agent_metric_cache') IS NOT NULL THEN
  ...
  END IF;
  IF to_regclass('public.store_marketing_profiles') IS NOT NULL THEN
  ...
  END IF;
  IF to_regclass('public.wechat_work_customers') IS NOT NULL THEN
    DROP INDEX IF EXISTS idx_ww_external_userid;
    CREATE UNIQUE INDEX IF NOT EXISTS idx_ww_external_userid ON wechat_work_customers (external_userid, tenant_id) WHERE external_userid IS NOT NULL AND external_userid <> '';
  END IF;
END $$;
