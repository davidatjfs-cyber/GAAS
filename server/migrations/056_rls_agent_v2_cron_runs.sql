...
DO $$
BEGIN
  IF to_regclass('public.agent_v2_cron_runs') IS NOT NULL THEN
    ALTER TABLE agent_v2_cron_runs ENABLE ROW LEVEL SECURITY;
    ALTER TABLE agent_v2_cron_runs FORCE ROW LEVEL SECURITY;
    DROP POLICY IF EXISTS tenant_isolation ON agent_v2_cron_runs;
    CREATE POLICY tenant_isolation ON agent_v2_cron_runs USING (tenant_id = current_setting('app.tenant_id', true)) WITH CHECK (tenant_id = current_setting('app.tenant_id', true));
  END IF;
END $$;
