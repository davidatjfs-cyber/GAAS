...
## 十八、开发纪律与已下线功能（2026-08-06 复盘补充）

### 开发纪律（强制）

1. **先价值验证，后开发**：任何新功能上线前，必须先拿真实数据证明有效性（相关性/命中率/可测量收益），验证不过就停，不进入 UI 开发；
2. **验证先行实验不打包上线**：基线/原型验证结果用于决策，不作为产品交付；
3. **系统里只保留"已经在用或有验证证据"的功能**：半成品、无法证明价值的功能必须下线或明确标记试验版，不得长期占用入口；
4. 复盘教训：菜品测试曾因"基线失败后改定位继续开发"而把无验证价值的规则版上线，用户使用后判定无意义——此错误不得重演。

### 已下线功能记录

- **菜品测试（客群匹配版，2026-08-06 下线）**：规则版匹配器（价格/辣度/场景 vs 固定原型）对真实点单热度无预测力（Spearman 0.22，价格 -0.07），输出等价于老板常识；前端入口与接口已从生产移除，代码保留在仓库（dish-test.js/dish-match.js/dish-risk.js/dish-checklist.js、frontend/src/pages/17-dish-test.js）供"真实分布采样版"改造参考，未经验证不得重新上线；
- 若重启菜品测试，唯一可接受形态：从 17,673 个真实食客画像采样 + 真实客单价锚定 + 已上架菜"预测 vs 真实"相关性 ≥0.6 验证通过后方可上线。
