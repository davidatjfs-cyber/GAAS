#!/usr/bin/env bash
# 安全部署 server 文件：拉生产对比 → 归档 bak → staged scp → md5 → pm2 reload → 健康检查
# Usage:
...
DRY_RUN="${DRY_RUN:-0}"
SSH=(ssh -o ConnectTimeout=45 -o ServerAliveInterval=5 "$HOST")
...
# 1) 远端准备 + 拉生产对比摘要
"${SSH[@]}" "mkdir -p '$BAK_REMOTE' '$STAGE_REMOTE' '$REMOTE/server'"
...
for f in "${FILES[@]}"; do
  ...
  if "${SSH[@]}" "test -f '$remote_path'"; then
    "${SCP[@]}" "$HOST:$remote_path" "$DIFF_DIR/$(basename "$f").prod" >/dev/null
...
for f in "${FILES[@]}"; do
  ...
  "${SSH[@]}" "mkdir -p '$remote_dir' && if [ -f '$remote_path' ]; then cp -a '$remote_path' '$BAK_REMOTE/'; fi"
  base="$(basename "$f")"
  "${SCP[@]}" "$f" "$HOST:$STAGE_REMOTE/$base"
done
...
for f in "${FILES[@]}"; do
  ...
  remote_md5="$("${SSH[@]}" "md5sum '$STAGE_REMOTE/$base' | awk '{print \$1}'")"
  if [[ "$local_md5" != "$remote_md5" ]]; then
    echo "MD5 mismatch staged $rel local=$local_md5 remote=$remote_md5"
    exit 1
  fi
  "${SSH[@]}" "mkdir -p '$(dirname "$remote_path")' && mv '$STAGE_REMOTE/$base' '$remote_path' && md5sum '$remote_path'"
  echo "  installed $rel md5=$local_md5"
done

# 4) syntax check key js + reload + health
CHECK_LIST=()
for f in "${FILES[@]}"; do
  case "$f" in
...
if [[ ${#CHECK_LIST[@]} -gt 0 ]]; then
  "${SSH[@]}" "for p in ${CHECK_LIST[*]}; do node --check \"\$p\"; done"
fi

echo "=== pm2 reload hrms-service ==="
"${SSH[@]}" 'pm2 reload hrms-service || pm2 restart hrms-service'
sleep 5
ROOT_CODE="$("${SSH[@]}" "curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:3000/")"
HEALTH_CODE="$("${SSH[@]}" "curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:3000/api/health")"
echo "ROOT=$ROOT_CODE HEALTH=$HEALTH_CODE"
if [[ "$ROOT_CODE" != "200" || "$HEALTH_CODE" != "200" ]]; then
  echo "健康检查失败。bak=$BAK_REMOTE"
  "${SSH[@]}" 'pm2 logs hrms-service --err --lines 20 --nostream' || true
  exit 1
fi
