...
### 🔴 反复复发类问题的判定与修法（2026-08-06，通知弹窗 5 周 4 次「根因修复」后总结）

**如果一个问题你已经修过第二次，不要再修第三次同样的地方——先判定它属于下面哪一类。**

真实案例：「通知重复弹窗」从 7/1 起复发 5 周，相关提交 **118 个**，其中 4 个标题写着
「根因修复 / 永久修复」（`ba8233f` / `c34fc21` / `48ebdaa` / `f52e78b`），每次都在几天内复发。
复盘发现每一轮修法都属于同一个类别，所以复发是必然的，不是运气问题：

```
进程内 Map 去重  → 重启清零
改成持久化       → 但每个 monitor 各写一份
加 10 分钟窗口   → 慢速重复绕过
放宽到未读       → ack 后锁就松开
加 4 小时冷却    → 并发下仍然双插
改成单条 SQL     → 注释写"原子"，实际不是
再补 3 处裸 INSERT → 下一个新增写入方照样绕过
```

**两种必然复发的形态，看到就要换类别修，不要再加一层守卫：**

1. **不变量靠「约定」而不是「约束」维护。**
   判据：这个规则是不是散落在 N 个代码路径里、靠每个人记得去遵守？
   如果是，维护成本是 O(代码路径数 × 未来改动数)，每个新写入方都是一次重新违约的机会。
   修法：**把不变量下沉到数据库约束**（UNIQUE / CHECK / 外键 / 部分唯一索引），
   让它不再是代码路径。本次即 migration 183 的
   `uniq_hrms_notif_unread_msg (lower(target_username), type, md5(message)) WHERE read_at IS NULL`。
   ⚠️ 特别注意：**`INSERT ... SELECT ... WHERE NOT EXISTS` 在 PostgreSQL 默认 READ COMMITTED 下
   不是原子判重**——两个并发事务互相看不到对方未提交的行，双方都会插入成功。
   没有 UNIQUE 约束时它挡不住并发重复，别再写「同一条 SQL 所以原子」这种注释。

2. **默认值给错了方向，靠黑名单往回收。**
   判据：新增一个同类东西时，它会不会**自动继承**某种危险能力（打断用户 / 全量权限 /
   自动执行），而添加者根本不知道有一份黑名单要同步维护？
   如果是，黑名单必然漏，因为漏的人不知道自己该维护它。
   修法：**反转默认值为白名单**——默认无害，想要那个能力必须显式登记。
   本次即 `FORCE_ACK_NOTIFICATION_TYPES`：默认所有通知都不能全屏拦人，
   只有备案类/审批结果类显式登记后才有资格。

**配套纪律**：
- 换类别修完后**必须加闸门**锁住新形态（如 `server/test/forced-ack-whitelist-gate.test.mjs`
  断言不能改回黑名单写法），否则下一个人图快又改回去。
- **禁止在提交标题里写「永久修复」「根因修复」**，除非你能说清楚"这次和前 N 次属于不同类别、
  为什么这次绕不过"。上面 4 个提交都这么写过，结果是让后来的人误以为这块已经解决了。
