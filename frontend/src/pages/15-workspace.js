...
function wsInjectStyles() {
    ...
    style.textContent =
        ...
        '.ws-br-filters{display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap;}' +
        '.ws-br-filters .ws-input{flex:1;min-width:100px;}' +
        '.ws-br-list{max-height:360px;overflow-y:auto;display:flex;flex-direction:column;gap:8px;}' +
        '.ws-br-item{background:var(--ws-card);border:1px solid var(--ws-line);border-radius:10px;padding:10px 12px;}' +
...
function wsRenderOverview(ov) {
    ...
    if (ov.turnover) {
        html += '<div class="ws-section" style="margin-top:14px;"><div class="ws-section__title">门店员工离职率（当月累计）</div>' +
            '<div class="ws-metrics-grid">' +
            '<div class="ws-metric"><div class="ws-metric__v">' + (ov.turnover.turnoverRate ?? '—') + '%</div><div class="ws-metric__l">离职率</div></div>' +
            '<div class="ws-metric"><div class="ws-metric__v">' + ov.turnover.departures + '</div><div class="ws-metric__l">离职人数</div></div>' +
            '<div class="ws-metric"><div class="ws-metric__v">' + ov.turnover.totalEmployees + '</div><div class="ws-metric__l">在职人数</div></div>' +
            '</div></div>';
    }
    ...
    if (Array.isArray(ov.team) && ov.team.length) {
        const teamRows = ov.team.slice(0, 10).map((t) => (
            ...
            '<span class="ws-rank-row__val">执行' + wsEsc(t.execution_rating || '-') + ' 态度' + wsEsc(t.attitude_rating || '-') + ' 能力' + wsEsc(t.ability_rating || '-') + '</span></div>'
        )).join('');
        html += '<div class="ws-section" style="margin-top:14px;"><div class="ws-section__title">下属绩效评级总览（本月）</div><div class="ws-rank-col">' + teamRows + '</div></div>';
    }
...
async function wsLoadBadReviews(container) {
    ...
    const start = container.querySelector('#ws-br-start')?.value || '';
    const end = container.querySelector('#ws-br-end')?.value || '';
    ...
    const items = Array.isArray(data?.items) ? data.items : [];
    const list = container.querySelector('#ws-br-list');
    if (!list) return;
    if (!items.length) { list.innerHTML = '<div class="ws-empty">没有符合条件的差评记录</div>'; return; }
...
function wsRenderBadReviewSection(allStores) {
    ...
    return (
        ...
        '<select class="ws-input" id="ws-br-store">' + storeOptions + '</select>' +
        '<input type="date" class="ws-input" id="ws-br-start" title="起始日期">' +
        '<input type="date" class="ws-input" id="ws-br-end" title="结束日期">' +
        '<button type="button" class="ws-btn ws-btn--primary" id="ws-br-search">检索</button>' +
        '</div>' +
        '<div class="ws-br-list" id="ws-br-list"><div class="ws-empty">加载中...</div></div>'
    );
}
...
async function wsRenderBossOrHq(root, persona) {
    ...
    const [home, insightsHtml, overview, marketingHtml] = await Promise.all([
    ...
    const brSearchBtn = root.querySelector('#ws-br-search');
    if (brSearchBtn) brSearchBtn.addEventListener('click', () => wsLoadBadReviews(root));
