...
async function loadDedupStats(){var el=document.getElementById('dedup-stats-content');if(!el)return;el.textContent='加载中...';try{var r=await fetch('/api/dedup/stats',{credentials:'include'});if(!r.ok)throw new Error('HTTP '+r.status);var d=await r.json();var t=d.tables||{};el.innerHTML='<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px 16px;font-size:13px;"><span>消息记录重复组:</span><span style="font-weight:600;color:'+(t.agent_messages_dup_groups>0?'#ef5350':'#66bb6a')+'">'+t.agent_messages_dup_groups+'</span><span>飞书通用记录:</span><span>'+t.feishu_generic_records+'</span><span>销售明细:</span><span>'+t.sales_raw+'</span><span>桌访记录:</span><span>'+t.table_visit_records+'</span></div>';}catch(e){el.textContent='加载失败: '+e.message;}}
...
async function deleteStoreDutyBinding(id){if(!id)return;var ok=await hrmsConfirm({title:'删除职责绑定',message:'确认删除这条职责绑定？',okText:'确认删除',icon:'🗑️'});if(!ok)return;try{await HRMS_API.deleteStoreDutyBinding(id);showNotification('职责绑定已删除','success');await loadStoreDutyBindings();}catch(e){showNotification('删除失败: '+(e?.message||e),'error');}}
        ...
        async function openAgentTaskDetail(taskId) {
            ...
            try {
                ...
                if (panel) panel.innerHTML = '<button class="atb-action" onclick="closeAgentTaskDetail()">关闭</button><div class="atb-section">加载失败：' + atbEscape(e.message) + '</div>';
            }
        }
        ...
        function renderAgentTaskDetail(t) {
            ...
            panel.innerHTML = '<div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;"><div><div class="atb-kicker">任务详情</div><h2 style="margin:6px 0 0;font-size:24px;line-height:1.15;">' + atbEscape(t.title || t.task_id) + '</h2></div><button class="atb-action" style="width:auto;padding:8px 12px;" onclick="closeAgentTaskDetail()">关闭</button></div>'
                ...
                + '<div class="atb-section"><b>重新分配负责人</b><select id="atb-reassign-agent" class="atb-input" style="margin-top:8px;width:100%;"><option value="">选择负责人...</option><option value="ops_supervisor">运营督导</option><option value="food_quality">食安专员</option><option value="train_advisor">培训顾问</option><option value="marketing_planner">营销策划</option><option value="marketing_executor">营销执行</option><option value="data_auditor">数据审计</option></select><button class="atb-action" style="margin-top:8px;width:100%;" onclick="reassignAgentTask()">确认重分配</button></div>'
                + '<div class="atb-section"><b>质量评分 (0-10)</b><input id="atb-quality-score" class="atb-input" type="number" min="0" max="10" step="0.5" style="margin-top:8px;width:100%;" placeholder="输入0-10的质量评分"><button class="atb-action" style="margin-top:8px;width:100%;" onclick="submitQualityScore()">提交评分</button></div>'
                + '<div class="atb-panel-actions"><button class="atb-action good" onclick="reviewAgentTask(\'approved\')">通过并关闭</button><button class="atb-action bad" onclick="reviewAgentTask(\'rejected\')">打回并修订</button><button class="atb-action" onclick="deriveAgentTask()">创建衍生任务</button></div>';
        }
        ...
        async function hrmsFetchPaymentSettingsAndBudgets() {
            ...
            const settings = hrmsNormalizePaymentSettings(data?.paymentSettings);
            const budgets = hrmsNormalizePaymentBudgets(data?.paymentBudgets);
            return { paymentSettings: settings, paymentBudgets: budgets };
        }
        ...
        async function hrmsSavePaymentSettingsAndBudgets(nextSettings, nextBudgets) {
            ...
            const st = await HRMS_API.getState();
            const base = (st && typeof st === 'object' ? (st.data || st) : {}) || {};
            const merged = {
                ...base,
                paymentSettings: hrmsNormalizePaymentSettings(nextSettings),
                paymentBudgets: hrmsNormalizePaymentBudgets(nextBudgets)
            };
            await HRMS_API.saveState(merged);
            try {
                const local = HRMS_STORE.ensure();
                local.paymentSettings = merged.paymentSettings;
                local.paymentBudgets = merged.paymentBudgets;
